> Original Source: plan/task-filter and contribution-list/2-implementation_plan.md
...
## Task-5: Global Contribution List (New Request)

### Database
#### [MODIFY] [js/db.js](file:///e:/Git_WIP/3.%20University%20repositories/b1t-Sched/js/db.js)
- Add `getAllActiveTasks()`: fetches all tasks where `status == 'active'` (no department filter).
    - *Note*: This might require a composite index if we sort, but for now just fetching `active` is enough.

### UI Changes
#### [MODIFY] [index.html](file:///e:/Git_WIP/3.%20University%20repositories/b1t-Sched/index.html)
- Add a toggle switch inside `.modal-header` or `.modal-body` of `#contributions-modal`.
    - Label: "Show All Departments"

#### [MODIFY] [css/dashboard.css](file:///e:/Git_WIP/3.%20University%20repositories/b1t-Sched/css/dashboard.css)
- Style the toggle switch.

#### [MODIFY] [js/app.js](file:///e:/Git_WIP/3.%20University%20repositories/b1t-Sched/js/app.js)
- Consolidate `showContributionsModal` logic.
- Add state variable `isGlobalContributions` (default false).
- When toggle changes:
    - If true: call `DB.getAllActiveTasks()`, aggregate, and re-render table.
    - If false: use `this.currentTasks`, aggregate, and re-render table.
- Cache global tasks if needed, or fetch every time (fetch is safer for real-time).

---

## Task-6: Documentation Update

#### [MODIFY] [doc/DOCUMENTATION.md](file:///e:/Git_WIP/3.%20University%20repositories/b1t-Sched/doc/DOCUMENTATION.md)
- **Key Features**: Add Task Filter, Global Contributions, User Counter.
- **UI Section**: Describe new UI elements.
- **API Reference**: Add `getAllActiveTasks`.
- **CSS Architecture**: Mention new styles in `dashboard.css`.
